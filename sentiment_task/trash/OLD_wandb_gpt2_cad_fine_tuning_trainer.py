...
def train(out_dir, model, trainset, valset):
...
def main():
    ...
    for fold in folds:
        ...
        # for training_cfg in train_grid:
            ...
            model_dir = f"{validation_path}fold_{fold}/gpt2-742/"

            # initialize WANDB logging system
            wandb.login()
            sweep_id = "cdiego89/counterfactual-generation/6q2or739"
            print(f"Sweep id:{sweep_id}")
            # wandb.agent(sweep_id, train(model_dir, lm, tokenized_train, tokenized_val), count=4)
            wandb.agent(sweep_id, function=train2, count=4)
            # wandb.agent(sweep_id, function=lambda: train(model_dir, lm, tokenized_train, tokenized_val), count=4)
