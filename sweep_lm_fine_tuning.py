...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path="train", block_size=512):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            # Diego dl = tokenizer(text, padding=True, truncation=True, return_tensors="pt")
    ...
    with wandb.init(settings=wandb.Settings(console='off')):
        ...
        if args.do_eval and args.local_rank in [-1, 0]:
            ...
            for checkpoint in checkpoints:
                ...
                print(f"perplexity:{result['perplexity_'].item()}")

                wandb.log({"perplexity": result['perplexity_'].item(),
